...
// Admin repo: push same codebase (admin panel is same app, different bot token)
if (ADMIN_TOKEN && ADMIN_REPO_URL) {
  ...
  try {
    ...
    const ignore = new Set(['.git', 'node_modules', 'attached_assets', '.local', '.upm', '.cache', 'db.json']);
