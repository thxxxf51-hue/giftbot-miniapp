/* ══════════════════════════════════════════════════════════
   BETS.JS — Ставки на спорт (API-Football)
══════════════════════════════════════════════════════════ */
...
function _odds(fix){
  ...
  return{o1:Math.max(1.2,+(o1+seed*.4-.2).toFixed(2)),ox:Math.max(2.5,+(ox+seed*.3-.15).toFixed(2)),o2:Math.max(1.2,+(o2+seed*.5-.25).toFixed(2))};
}
...
function _card(fix, hi){
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">${statusHtml}<div style="font-size:9px;color:rgba(255,255,255,.18);font-weight:600">${live&&!pause?'идёт игра':pause?'перерыв':''}</div></div>
...
function _league(lid, name, matches, idx){
  const isLive=_bTab==='live';
  const liveBadge=isLive?`<div class="bets-live-badge"><span class="bets-dot-live" style="width:5px;height:5px"></span>LIVE</div>`:'';
...
async function betsLoadLive(){
  ...
  try{
    const d=await fetch('/api/sports/live').then(r=>r.json());
    const fixtures=d.fixtures||[];
    const liveEl=document.getElementById('bets-live-count');
    if(liveEl) liveEl.textContent=fixtures.length+' матчей live';
    if(!fixtures.length){ el.innerHTML=`<div class="bets-empty">⚽<br>Нет live матчей</div>`; return; }
    const lg={};
...
async function betsLoadToday(){
  ...
  el.innerHTML=`<div class="bets-loading">⚽ Загружаем...</div>`;
  try{
    const d=await fetch('/api/sports/today').then(r=>r.json());
    const fixtures=d.fixtures||[];
    if(!fixtures.length){ el.innerHTML=`<div class="bets-empty">📅<br>Нет матчей на сегодня</div>`; return; }
    const lg={};
    fixtures.forEach(f=>{ const id=f.league.id; if(!lg[id]) lg[id]={name:f.league.name,m:[]}; lg[id].m.push(f); });
    let html='', idx=0;
    Object.entries(lg).forEach(([id,g])=>{ html+=_league(+id,g.name,g.m,idx); idx+=g.m.length; });
...
async function betsLoadHistory(){
  ...
  try{
    ...
    const bets=d.bets||[];
    if(!bets.length){ el.innerHTML=`<div class="bets-empty">${COIN_SVG_Y}<br>Ставок пока нет</div>`; return; }
...
function betsSwitchTab(name, el){
  ...
  ['live','today','history'].forEach(t=>{
    const el2=document.getElementById('bets-tab-'+t);
...
function onBetsPageEnter(){
  ...
  document.getElementById('bets-tab-live').style.display='';
  document.getElementById('bets-tab-today').style.display='none';
  document.getElementById('bets-tab-history').style.display='none';
  betsLoadLive();
...
function betsOpenSlip(card, matchName, o1, ox, o2, home, away){
  _bMatch=matchName; _bOdds=o1; _betsPick='П1 ('+home+')'; _bCur='coins';
...
function betsPickOdd(btn){
  ...
  if(idx>=0){
    const slipBtn=document.querySelectorAll('.bsodd')[idx];
    if(slipBtn){ _bOdds=parseFloat(slipBtn.dataset.odds); _betsPick=slipBtn.dataset.label; document.getElementById('bs-pick').textContent=_betsPick; document.getElementById('bs-odds-badge').textContent='× '+_bOdds.toFixed(2); }
  }
  betsCalcPot();
}
...
function betsSlipPick(btn){
  ...
  _betsPick=btn.dataset.label;
  document.getElementById('bs-pick').textContent=_betsPick;
...
async function betsSubmit(){
  ...
  try{
    const d=await fetch('/api/bets/place',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uid,matchName:_bMatch,pick:_betsPick,odds:_bOdds,amount:amt,currency:_bCur})}).then(r=>r.json());
