...
function _minesUpdateUI() {
  ...
  if (profit) {
    profit.textContent = _minesOpened > 0 ? '+' + Math.round(_minesBet * _minesMult).toLocaleString('ru') + ' 🪙' : '—';
  }
  // cashout btn
...
function _minesShowResult(win, amount, perfect) {
  ...
  document.getElementById('mines-result-title').textContent = win
    ? (perfect ? 'Идеальная игра!' : '+' + amount.toLocaleString('ru') + ' 🪙')
    : 'Взрыв!';
