...
function rShopItems(){
  ...
  function _buildCustomItemHtml(x){
    ...
    const cntHtml=stockLeft!==null?`<div style="position:absolute;top:6px;right:6px;z-index:5;background:rgba(0,0,0,0.65);color:#fff;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px;pointer-events:none;white-space:nowrap">${stockLeft>0?stockLeft+' шт.':'Нет'}</div>`:(x.count?`<div style="position:absolute;top:6px;right:6px;z-index:5;background:rgba(0,0,0,0.65);color:#fff;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px;pointer-events:none;white-space:nowrap">${x.count} шт.</div>`:'');
    ...
    return`<div class="sitem" ${borderStyle} onclick="openShopModal('custom',${x.id})">
      <div class="sitem-img-wrap" style="overflow:visible;position:relative">${tagHtml}${cntHtml}${imgContent}</div>
      <div class="sitem-body">
        <div class="sname">${x.name}</div>
        ${x.desc?`<div class="sitem-desc">${x.desc}</div>`:''}
      </div>
...
function buyCustomItem(id){
  ...
  const stockLeft=x.stock!==null&&x.stock!==undefined?Number(x.stock):null;
  if(S.balance<x.price){toast('Недостаточно монет','r');return;}
  if(stockLeft!==null&&stockLeft<=0){toast('Нет в наличии','r');return;}
  openGenMo(`Купить ${x.name}?`,x.desc||`Спишется ${x.price} монет`,`🛒 Купить — ${x.price} <svg viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:13px;height:13px;vertical-align:-2px;flex-shrink:0"><circle cx="8" cy="8" r="7"/><path d="M19.5 9.94a7 7 0 11-9.56 9.56"/><path d="M7 6h1v4"/><path d="M17.3 14.3l.7.7-2.8 2.8"/></svg>`,async()=>{
    closeGenMo();
    try{
      const r=await fetch('/api/shop/buy-custom',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:UID,itemId:id})});
