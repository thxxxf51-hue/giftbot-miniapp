...
let _soloCanvas=null,_soloCtx=null,_soloRaf=null;
let _soloRot=0,_soloSpinning=false;
let _soloPrizeIdx=0,_soloChanceIdx=4; // default 10%
...
function _soloDrawWheel(canvas,chancePct,prizeIdx,rotation){
  ...
  const R=Math.min(W,H)/2-6,innerR=R*0.36;
  ...
  ctx.arc(cx,cy,R,winEnd,winStart+Math.PI*2);ctx.closePath();
  const lg=ctx.createRadialGradient(cx,cy,0,cx,cy,R);
  lg.addColorStop(0,'#1a1a24');lg.addColorStop(1,'#14141c');
  ctx.fillStyle=lg;ctx.fill();

  // No Loot label
  const loseAngle=Math.PI*2-winAngle;
  const midLose=winEnd+loseAngle/2;
  ctx.save();
  ctx.translate(cx+Math.cos(midLose)*R*0.65,cy+Math.sin(midLose)*R*0.65);
  ctx.rotate(midLose+Math.PI/2);
  ctx.fillStyle='rgba(255,255,255,0.1)';
  ctx.font=`600 ${R*0.085}px -apple-system,sans-serif`;
  ctx.textAlign='center';ctx.fillText('No Loot',0,0);
  ctx.restore();

  // win sector
  const [c1,c2]=soloColor(prizeIdx);
  const midWin=winStart+winAngle/2;
  const wg=ctx.createLinearGradient(
    cx+Math.cos(winStart+0.1)*R*0.6,cy+Math.sin(winStart+0.1)*R*0.6,
    cx+Math.cos(winEnd-0.1)*R*0.6,cy+Math.sin(winEnd-0.1)*R*0.6
  );
  wg.addColorStop(0,c1);wg.addColorStop(1,c2);
  ctx.beginPath();ctx.moveTo(cx,cy);ctx.arc(cx,cy,R,winStart,winEnd);ctx.closePath();
  ...
  if(chancePct>=10){
    ctx.save();
    ctx.translate(cx+Math.cos(midWin)*R*0.62,cy+Math.sin(midWin)*R*0.62);
    ctx.rotate(midWin+Math.PI/2);
    ctx.fillStyle='rgba(255,255,255,0.95)';
    ctx.font=`bold ${Math.min(R*0.095,14)}px -apple-system,sans-serif`;
    ctx.textAlign='center';
    const p=SOLO_PRIZES[prizeIdx];
    ctx.fillText(p>=1000?(p/1000)+'K 🪙':p+' 🪙',0,0);
    ctx.restore();
  }
  ...
  // center text
  ctx.fillStyle=_soloSpinning?'rgba(255,255,255,0.3)':c1;
  ctx.font=`900 ${Math.min(innerR*0.55,18)}px -apple-system,sans-serif`;
  ctx.textAlign='center';ctx.textBaseline='middle';
  ctx.fillText(_soloSpinning?'...':`${chancePct}%`,cx,cy-3);
  ctx.fillStyle='rgba(255,255,255,0.25)';
  ctx.font=`600 ${Math.min(innerR*0.28,9)}px -apple-system,sans-serif`;
  ctx.fillText(_soloSpinning?'':'ШАНС',cx,cy+innerR*0.38);
...
function soloRenderUI(){
  const pct=SOLO_CHANCE_STEPS[_soloChanceIdx];
  const prize=SOLO_PRIZES[_soloPrizeIdx];
  const cost=soloCost(prize,pct);
  const [c1]=soloColor(_soloPrizeIdx);
  const steps=SOLO_CHANCE_STEPS.length;
  ...
  const thumbs=document.querySelectorAll('.solo-dot');
  thumbs.forEach((d,i)=>{
    const isSel=i===_soloChanceIdx;
    d.style.width=isSel?'22px':'8px';
    d.style.height=isSel?'22px':'8px';
  ...
  if(btn){
    btn.textContent=_soloSpinning?'⏳ Крутится...':`Крутить за ${cost.toLocaleString('ru')} 🪙`;
  ...
  // prize button
  const pbtn=document.getElementById('solo-prize-btn');
  if(pbtn){
    pbtn.querySelector('#solo-prize-val').textContent=prize.toLocaleString('ru')+' 🪙';
    pbtn.querySelector('#solo-prize-val').style.color=c1;
  }
...
function initSoloPage(){
  ...
  if(grid&&!grid.children.length){
    SOLO_PRIZES.forEach((p,i)=>{
      ...
      btn.className='solo-pgrid-btn';
      btn.dataset.idx=i;
      btn.innerHTML=`<span>${p>=1000?(p/1000)+'K':p}</span><span style="font-size:9px;opacity:.5">🪙</span>`;
      btn.onclick=()=>{
        _soloPrizeIdx=i;
        document.getElementById('solo-prize-picker').style.display='none';
        soloRenderUI();
      };
...
function soloSpin(){
  ...
  _soloSpinning=true;
  document.getElementById('solo-prize-picker')?.style&&(document.getElementById('solo-prize-picker').style.display='none');
  soloRenderUI();
  ...
  let targetOffset;
  if(win){ targetOffset=winAngle*(0.25+Math.random()*0.5); }
  else   { const la=Math.PI*2-winAngle; targetOffset=winAngle+la*(0.1+Math.random()*0.8); }
  ...
  function frame(now){
    ...
    if(t<1){requestAnimationFrame(frame);}
      ...
      soloRenderUI();
      // show toast like promo
      const msg=win?`+${prize.toLocaleString('ru')} 🪙 Победа!`:'❌ Не повезло';
      toast(msg, win?'g':'r', win
...
/* ── prize picker toggle ── */
function soloTogglePrize(){
  const el=document.getElementById('solo-prize-picker');
  if(!el)return;
  ...
  document.querySelectorAll('.solo-pgrid-btn').forEach((b,i)=>{
    const [pc]=soloColor(i);
    b.style.background=i===_soloPrizeIdx?`${pc}1a`:'rgba(255,255,255,.04)';
    b.style.border=`1px solid ${i===_soloPrizeIdx?pc:'rgba(255,255,255,.07)'}`;
    b.style.color=i===_soloPrizeIdx?pc:'rgba(255,255,255,.55)';
  });
}
